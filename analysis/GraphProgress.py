...
if __name__ == '__main__':
    ...
    smooth_is_crashed = running_avg(is_crashed,window_size)
    #smooth_is_success = savgol_filter(is_success, window_length=window_size, polyorder=3)
    smooth_is_success = running_avg(is_success,window_size)
    ...
    ax1 = fig.add_subplot(111)
    ax1.plot(df['ep_num'].values, smooth_is_crashed, color='tab:blue')
    ax1.plot(df['ep_num'].values, is_crashed, '--', alpha=0.3, color='tab:blue')

    # graph std tolerance
    polygon = Polygon(std_poly_is_crashed, True, label='Std')
    bp = [polygon]
    ...
    ax1 = fig.add_subplot(111)
    ax1.plot(df['ep_num'].values, smooth_is_success, color='tab:blue')
    ax1.plot(df['ep_num'].values, is_success, '--', alpha=0.3, color='tab:blue')

    # graph std tolerance
    polygon = Polygon(std_poly_is_success, True, label='Std')
    bp = [polygon]
